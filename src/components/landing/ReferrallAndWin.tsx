    ...
    >
      ...
      <div className="relative z-10 flex flex-col md:flex-row justify-between max-w-6xl items-center px-[24px] bg-[#350b2d] w-full md:gap-x-[32px] gap-x-0">
        ...
        <div className="flex flex-col gap-y-[32px] w-full md:w-1/2">
          <div className="relative md:min-w-2xl w-full">
            ...
            <span className="md:text-[46px] text-[30px] font-[700] capitalize leading-[130%] relative z-10">
              {t("referral.title")}
            </span>
          </div>

          <span className="text-[19px]">{t("referral.desc", { percent })}</span>
      ...
      <style>{`
        ...
        @keyframes ruletaFloat {
          0%   { transform: translateX(0) translateY(0); opacity: 0.98; }
          18%  { transform: translateX(calc(var(--x-amp) * -0.55)) translateY(-10px); opacity: 1; }
          40%  { transform: translateX(calc(var(--x-amp) * 0.9)) translateY(6px);  opacity: 0.96; }
          66%  { transform: translateX(calc(var(--x-amp) * -0.28)) translateY(-6px); opacity: 0.99; }
          100% { transform: translateX(0) translateY(0); opacity: 0.98; }
        }
        ...
        @keyframes rise {
          ...
          10%  { opacity: 0.9; }
          60%  { transform: translateY(-70vh) translateX(var(--x-offset)); opacity: 0.35; }
          100% { transform: translateY(-140vh) translateX(calc(var(--x-offset) * 1.2)); opacity: 0; }
        }

        @media (prefers-reduced-motion: reduce) {
          .floating-card, .ruleta-wrapper, .ruleta-no-rotate, .particle {
